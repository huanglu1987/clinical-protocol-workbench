...
| `assets/strategy-dependencies.lock.json` | 固定两个皮肤科策略 Skill 的提交和受控文件校验码（SHA-256） | 只读版本清单 |
...
| 编号 | 含义 | 能否进入正文 |
| --- | --- | --- |
| `SRC-*` | 输入来源及其版本、文件校验码、角色和允许用途 | 视角色而定 |
...
1. **锁定来源**：读取所有提供文件，记录版本并计算文件校验码，登记每份资料能支持什么、不能支持什么。
